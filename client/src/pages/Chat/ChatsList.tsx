...
const ChatList = () => {
  ...
  const { data: chatData, isLoading: isLoadingChats, error: errorChats } = useFetchChatsByUserIdQuery(userInfo._id);

  useEffect(() => {
    if (chatData?.chats && chatData.chats.length > 0) {
      const sortedChats = [...chatData.chats].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      dispatch(setCurrentChat(sortedChats[0])); // Set the most recent chat
    }
  ...
  const { data: usersData } = useFetchUsersByIdsQuery(memberIds, { skip: memberIds.length === 0 });
        ...
        {chatData?.chats.map(chat => (
          ...
          >
            ...
            <div className="flex-1">
              <p>
                {chat.members.filter(id => id !== userInfo._id).map(id => users[id]?.name || "Unknown User").join(", ")}
              </p>
            ...
            <div className="text-right">
              <p className="text-sm text-gray-500">
                {formatDate(lastMessages[chat._id]?.createdAt ?? chat.createdAt)}
              </p>
            </div>
          </div>
        ))}
